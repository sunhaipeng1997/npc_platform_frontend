				...
				<div class="block">
					<el-row>
						<div>
							<el-col :span="4"
							        :offset="8">
								<p class="demonstration ">设置建议接收人员:
							...
							<el-col :span="8">
								<el-cascader
										...
										@change="setUids"
										v-model="modalAdvice"
										@visible-change="handleChange(0,$event)"
										@remove-tag="handleRemove(0,$event)"
								></el-cascader>
							</el-col>
						</div>
					</el-row>
					<el-row>
						<div>
							<el-col :span="4"
							        :offset="8">
								<p class="demonstration ">设置通知审核人员:
							...
							<el-col :span="8">
								<el-cascader
										...
										:props="props"
										@change="setUids"
										v-model="modalNotification"
										@visible-change="handleChange(1,$event)"
										@remove-tag="handleRemove(1,$event)"
					...
					<el-row>
						<div>
							<el-col :span="4"
							        :offset="8">
								<p class="demonstration ">设置履职登记总审核人:
							...
							<el-col :span="8">
								<el-cascader
										...
										:props="props"
										@change="setUids"
										v-model="modalPerformance"
										@visible-change="handleChange(3,$event)"
										@remove-tag="handleRemove(3,$event)"
					...
					<el-row>
						<div>
							<el-col :span="4"
							        :offset="8">
								<p class="demonstration">是否开启小组审核人员</p></el-col>
							<el-col :span="8">
								<el-switch
										style="margin-top: 10px;"
										v-model="isGroupAudit"
										inactive-text="关闭"
										active-text="开启"
										@change="handleSwitch"
								>
					...
					<el-row style="margin-top: -10px;">
						<el-col :span="4"
						        :offset="8">
							<p class="demonstration ">设置履职登记审核人员</p>
						</el-col>
						<el-col :span="8">
							<el-button v-for="(item,index) in options"
							           type="primary"
							           :disabled="!isGroupAudit"
							           @click="showForm(index)">{{item.name}}
							</el-button>
						</el-col>
						<el-col :span="4"
						        :offset="8">
						...
						<el-col :span="8">
							<p v-for="(item,index) in settingConfig.GROUPS">{{item.NAME}}: {{item.AUDITOR_NAME}}</p>
						</el-col>
					</el-row>
...
<script>
	export default {
		...
		methods: {
			...
			// 设置建议接收人
			setAdviceReveiver(times) {
				this.tableLoading = true;
				setTimeout(() => {
					this.$request.httpRequest({
						method: 'post',
						url: "/member_house/special_function/adviceReceiver",
						noLoading: true,
						params: {
							uids: this.uids
						},
			...
			// 设置通知审核人
			setNotificationAuditor(times) {
				this.tableLoading = true;
				setTimeout(() => {
					this.$request.httpRequest({
						method: 'post',
						url: "/member_house/special_function/notificationAuditor",
						noLoading: true,
						params: {
							uids: this.uids
						},
			...
			// 设置履职登记审核人
			setPerformanceAuditorManager(times) {
				this.tableLoading = true;
				setTimeout(() => {
					this.$request.httpRequest({
						method: 'post',
						url: "/member_house/special_function/performanceAuditorManager",
						noLoading: true,
						params: {
							uids: this.uids
						},
			...
			// 切换审核人员开关
			setAuditorSwitcher(times) {
				this.tableLoading = true;
				setTimeout(() => {
					this.$request.httpRequest({
						method: 'post',
						url: "/member_house/special_function/auditorSwitch",
						noLoading: true,
						params: {
							switches: this.isGroupAudit
						},
						success: data => {
							this.tableLoading = false;
							if (times == 0) {
			...
			// 设置履职镇审核人
			setPerformanceTownAuditor(times) {
				this.tableLoading = true;
				setTimeout(() => {
					this.$request.httpRequest({
						method: 'post',
						url: "/member_house/special_function/performanceTownAuditor",
						noLoading: true,
						params: {
							town: this.form.townUid,
							uid: this.form.personUid[0]
						},
						success: data => {
							this.tableLoading = false;
							this.getSettings();
