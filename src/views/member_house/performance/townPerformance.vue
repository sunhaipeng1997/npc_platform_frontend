								...
								<el-row :gutter="24">
									<el-col :span="5">
										<el-form-item label="镇/街道">
											<el-select v-model="searchForm.townUid"
													   placeholder="请选择下属镇"
													   @change="getTypeList"
													   size="mini">
												<el-option label="全部"
														   value=""></el-option>
												<el-option v-for="item in subTownsList"
														   :key="item.uid"
														   :label="item.name"
								...
								<el-row :gutter="24">
									...
									<el-col :span="5">
										<el-date-picker type="date"
														...
														size="mini"
														placeholder="选择结束日期"
														v-model="searchForm.dateEnd"></el-date-picker>
									</el-col>
								</el-row>
							</el-col>
						</el-row>
			...
			getTypeList(item) {
			  if (item) {
                this.$request.httpRequest({
                  method: 'get',
                  url: "/member_house/performance/subTownPerformanceTypeList",
                  params: {
                    townUid: item
                  },
                  noLoading: true,
                  ...
                  success: data => {
                    this.typeList = data;
                    let temp = this.typeList[1];
                    this.typeList[1] = this.typeList[this.typeList.length - 1];
                    this.typeList[this.typeList.length - 1] = temp;
                  },
                  error: e => {
                    this.$message({
                      type: 'warning',
			  ...
			  getSubTownsList() {
				this.$request.httpRequest({
				  method: 'get',
				  url: "/manager/town/subTownsList",
				  noLoading: true,
				  returnFullData: false,
				  success: data => {
					this.subTownsList = data;
				  },
				  error: e => {
					this.$message({
					  type: 'warning',
			...
			getData(times,event) {
				...
				setTimeout(() => {
					this.$request.httpRequest({
						...
						params: {
						  	flag: false,
                          	townUid: this.searchForm.townUid,
