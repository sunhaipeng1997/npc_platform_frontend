...
<!--                <el-button type="primary" size="mini" @click="">导出</el-button>-->
        ...
        <el-table v-loading="tableLoading" border size="mini" :header-row-style="{ 'font-size':'15px'}" :data="tableData" style="width: 100%" class="table-content" :row-class-name="tableRowClass">
          ...
          <el-table-column prop="expectDate" label="预计办完时间"  width="165" align="center">
...
<!--              <el-badge value="期" is-dot style="top: 5px;" class="item">{{scope.row.expectDate}}</el-badge>-->
          ...
          <el-table-column label="操作" width="230" align="center" class-name="operation">
            <template slot-scope="scope">
              <a v-if="$route.meta.edit" class="item" @click="urgeSug(scope.row)">催办</a>
              <a v-if="$route.meta.delete" class="item" @click="suggestionDetail(scope.row)">查看详情</a>
            </template>
          </el-table-column>
        </el-table>
      </template>
...
<script>
  ...
  computed:{
      ...
      methods: {
        tableRowClass({row}) {
            if (row.urge || row.exceedLimit) {
                return 'text-danger';
            } else if (row.closeDeadLine) {
                return 'text-warning';
            }
        },
      ...
      urgeSug(row) {
          this.$confirm('确认催办建议？')
              .then(_ => {
                  this.$request.httpRequest({
                      method: 'post',
                      url: '/suggestion_deal/gov_suggestion/urgeSug',
                      noLoading: true,
                      params: {
                          uid:row.uid,
                      },
