                                ...
                                <el-row :gutter="20">
                                    <el-col :span="6">
                                        <el-form-item label="名称">
                                            <el-input placeholder="请输业务名称" size="mini"
    ...
    export default {
        ...
        methods: {
            ...
            getData(times,event) {
                ...
                setTimeout(() => {
                    this.$request.httpRequest({
                        method: 'get',
                        url: "/suggestion_deal/npc_suggestion/suggestionBusiness",
                        noLoading: true,
                        params: {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        that.$request.httpRequest({
                            method: 'post',
                            url: "/suggestion_deal/npc_suggestion/addOrUpdateSuggestionBusiness",
                            noLoading: true,
                            params: {
                                uid: that.form.uid,
                                name: that.form.name,
            ...
            changeStatus(row, status) {
                this.$request.httpRequest({
                    method: 'post',
                    url: "/suggestion_deal/npc_suggestion/changeBusinessStatus",
                    noLoading: true,
                    params: {
                        uid: row.uid,
                        status: status
            ...
            changeSequence(row, type) {
                this.$request.httpRequest({
                    method: 'post',
                    url: "/suggestion_deal/npc_suggestion/changeTypeSequence",
                    noLoading: true,
                    params: {
                        uid: row.uid,
                        type: type
            ...
            deleteType(row) {
                this.$confirm('确认删除该类型吗?', '提示', {
                    ...
                    this.$request.httpRequest({
                        method: 'delete',
                        url: "/suggestion_deal/npc_suggestion/deleteSuggestionBusiness",
                        noLoading: true,
                        params: {
                            uid: row.uid
                        },
