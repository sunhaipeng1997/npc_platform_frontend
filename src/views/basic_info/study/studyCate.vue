				...
				<el-table v-loading="tableLoading"
					...
					<el-table-column label="操作"
						...
						<template slot-scope="scope">
							...
							<a v-if="$route.meta.edit && scope.row.status == 2"
							   class="item-warning"
							   @click="changeStatus(scope.row,1)">启用</a>
							<a v-if="$route.meta.edit && scope.row.status == 1"
							   class="item-success"
							   @click="changeStatus(scope.row,2)">停用</a>
							<a v-if="$route.meta.delete"
							   class="item-danger"
							   @click="deleteType(scope.row)">删除</a>
						</template>
					</el-table-column>
				</el-table>
	...
	export default {
		...
		methods: {
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						that.$request.httpRequest({
							method: 'post',
							url: "/manager/study/addOrUpdateStudyType",
							noLoading: true,
							params: {
								uid: that.form.uid,
								name: that.form.name,
			...
			changeStatus(row, status) {
				this.$request.httpRequest({
					method: 'post',
					url: "/manager/study/changeTypeStatus",
					noLoading: true,
					params: {
						uid: row.uid,
						status: status,
			...
			changeSequence(row, type) {
				this.$request.httpRequest({
					method: 'post',
					url: "/manager/study/changeTypeSequence",
					noLoading: true,
					params: {
						uid: row.uid,
						type: type,
			...
			deleteType(row) {
				this.$confirm('确认删除该类型吗?', '提示', {
					...
					.then(() => {
						this.$request.httpRequest({
							method: 'delete',
							url: "/manager/study/deleteStudyType",
							noLoading: true,
							params: {
								uid: row.uid,
							},
