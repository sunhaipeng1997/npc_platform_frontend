...
const permission = {
  ...
  actions: {
    ...
    GenerateRoutes({ commit }, isSuperAdmin) {
      resetRouter(); // 先初始化路由
      let role = JSON.parse(localStorage.getItem('vuex')).user.roles[0];
      return new Promise((resolve, reject) => {
        ...
        if (isSuperAdmin) {
          ...
          accessedRoutes.forEach(item => {
            if (item.children) {
              // 超级管理员赋全部权限
              //这里的超级管理员主要是界面操作权限，我们默认都有
              if (item.name === 'suggestionDeal'){
                if (role === 'GOVERNMENT'){
                    item.redirect = 'suggestion_deal/gov/govDealIndex';
                }
                if (role === 'UNIT'){
                  item.redirect = 'suggestion_deal/unit/unitDealIndex';
                }
                if (role === 'BACKGROUND_ADMIN'){
                  item.redirect = 'suggestion_deal/npc/npcDealIndex';
                }
              }
